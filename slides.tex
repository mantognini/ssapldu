...
% A (blue) cons cell with a circle in the "cdr" box.  Arguments are
% the same as for \cell.
\newcommand{\cons}[3][]{%
  \cell[#1]{#2}{#3}{blue}%
  \node[circle, fill=black, inner sep=.75pt] at (#2-cdr) {};}%

% A shared cons cell (drawn in red).
\newcommand{\scons}[3][]{%
  \cell[#1]{#2}{#3}{red}%
  \node[circle, fill=black, inner sep=.75pt] at (#2-cdr) {};}%

% Variable label and edge. Arguments are position, name and label text.
\newcommand{\vlabel}[3]{%
  \node[labelnode, #1=of #2] {$#3$} edge (#2);}%

\ifcompileTreeSlides
\begin{frame}[fragile,t]{Graph example}
  \vspace{-0.5em}\uncover<2-4>{
  \begin{center}
    \begin{tikzpicture}[semithick,
      ...
      % Matrix for arranging graph nodes.  You can add extra
      % horizontal or vertical space in brackets after \& and \\.
      \matrix[graph, level 2] (g1) %
      {
        \cons[minimum width=1em]{x}{\{x\}} \& \\
        \cons{xt1}{\{x, t_1\}} \& \scons{e}{\emptyset}\\[.5em] % <-- extra space
        \cons{y}{\{y\}} \& \cons{t}{\{t\}}\\
      };
      ...
      (xt1-cdr) edge (e)
      (y-cdr) edge (t);
      \draw[->] (e-cdr) -| ++(1em, 1em) -| (e.north);  % Loop.
